epochs=$1
mkdir resutls/bleu/$src-$tgt
mkdir resutls/train/$src-$tgt
  ...
  "
    ...
    python train.py \
      ...
      --tensorboard-logdir log \
      --log-format json | tee resutls/train/$src-$tgt-$num.log | grep valid_bleu | tee resutls/bleu/$src-$tgt/bleu_$src-$tgt-$num.log
